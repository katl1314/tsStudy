...
function addToType(a: number, b: number): number {
    return a + b;
}
...
// 함수 타입
type myFunction2 = (x: number, y: number) => number;

// 타입스크립트는 Typescript컴파일러가 방정식의 한쪽에만 타입이 명시되어 있더라도 타입을 추론할 수 있음.
let myAdd3: myFunction2 = function (x, y) {
    return x + y;
};
...
function buileName2(firstName: string, lastName?: string): string {
    if (lastName) {
        return `${firstName} ${lastName}`;
    }
    return firstName;
...
function buildName3(firstName: string, lastName: string = "Kim"): string {
    return lastName + " " + firstName;
}

// 나머지 매개변수
// 매개변수 마지막 위치에 rest문법을 이용하여 나머지 매개변수를 하나의 변수로 받을 수 있음.
function buildName4(firstName: string, ...lastName: string[]): string {
    // Array.prototype.join(sep): 배열요소를 연결하여 새로운 문자열로 만듦.
    return `${firstName} ${lastName.join(" ")}`;
}

// 함수 타입
type FnType = (firstName: string, ...lastName: string[]) => string;
const buildNameFn: FnType = buildName4;

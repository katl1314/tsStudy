...
console.log(genericIdentity(["apple", "graph"])); // 함수 호출시 타입 변수에 값을 전달. 함수 매개변수와 반환값 타입은 string
/**
...
let numIdentity = genericIdentity;
console.log(numIdentity([1, 2, 3]));
// 제네릭 클래스
class GenericNumber {
}
const myGeneratorNumber = new GenericNumber();
...
loggingIdentity("apple"); // 문자열을 객체로서, length프로퍼티를 갖는다. length속성 사용시 잠시 래퍼 객체 생성
...
hello(1);
hello("as");
function getPersonInfo(obj, key) {
    return key in obj;
}
const myPerson = {
    name: "James",
